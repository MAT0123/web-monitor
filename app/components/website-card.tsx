...
export function WebsiteCard({ website, onUpdate }: WebsiteCardProps) {
  ...
  const getUptimePercentage = () => {
    if (!website.recent_checks || website.recent_checks.length === 0)
      return 'N/A';

    const upChecks = website.recent_checks.filter(
  ...
  return (
    <Card
      className={`transition-all ${!website.is_active ? 'opacity-60' : ''}`}
    >
